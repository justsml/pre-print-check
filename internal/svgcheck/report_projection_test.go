...
func TestProjectReportBuildsPortableContract(t *testing.T) {
	report, err := Check([]byte(`<svg width="100" height="50"><script>alert(1)</script><rect onclick="x()"/></svg>`), "vinyl")
	if err != nil {
		t.Fatalf("Check returned error: %v", err)
	}

	portable := ProjectReport(report)
...
func TestPortableReportJSONContract(t *testing.T) {
	report, err := Check([]byte(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10"><path d="M 0 0 L 1 1" stroke="black" stroke-width="0.2"/></svg>`), "paper@10in")
	if err != nil {
		t.Fatalf("Check returned error: %v", err)
	}

	body, err := json.Marshal(ProjectReport(report))
	if err != nil {
